...
const raceCountryName = aliasCountryName(data.race.meetingCountryName).toLowerCase().replace(/\s/g, "-");
const flagUrl = "https://www.formula1.com/content/dam/fom-website/2018-redesign-assets/Flags%2016x9/" + raceCountryName + "-flag.png";
...
const [logo, flag, circuit] = await Promise.all([
  getImage("https://calikasten.wordpress.com/wp-content/uploads/2025/04/f1-logo.png"),
  getImage(flagUrl),
  getImage(circuitUrl),
]);
...
function getNextEvent(events) {
  ...
  for (let event of events) {
    const eventDate = new Date(Date.parse(event.startTime) + event.gmtOffset * 1000);
